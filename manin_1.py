...
Button(login_frame, text="Login", command=handle_login).pack(pady=10)

# Main Application Frame
main_frame = tk.Frame(root)

# Main GUI Components
Button(main_frame, text="Load Model Answer Sheet", command=select_model_answer).pack(pady=5)
model_answer_label = Label(main_frame, text="No file selected", fg="gray")
model_answer_label.pack()

Button(main_frame, text="Generate Metadata", command=generate_metadata_button).pack(pady=5)
generate_metadata_label = Label(main_frame, text="No metadata generated", fg="gray")
generate_metadata_label.pack()

Button(main_frame, text="Load Metadata File", command=select_metadata).pack(pady=5)
load_metadata_label = Label(main_frame, text="No file selected", fg="gray")
load_metadata_label.pack()

Button(main_frame, text="Select Student Folder", command=select_student_folder).pack(pady=5)
student_folder_label = Label(main_frame, text="No folder selected", fg="gray")
student_folder_label.pack()

Label(main_frame, text="Select Output Format:").pack(pady=5)
Radiobutton(main_frame, text="CSV", variable=output_format, value="csv").pack()
Radiobutton(main_frame, text="XLSX", variable=output_format, value="xlsx").pack()

Button(main_frame, text="Select Output File", command=save_output_file).pack(pady=5)
output_file_label = Label(main_frame, text="No file selected", fg="gray")
output_file_label.pack()

Button(main_frame, text="Run Grading", command=run_grading).pack(pady=20)
